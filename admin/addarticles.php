...
<?php 
  if(isset($_REQUEST['submit']))
  {
   ...
   $email = $_SESSION['email'];
    ...
    if($title & $content & $keyword & $description){
    $len = $_REQUEST['len'];
    if ($_FILES['file']['name']) {$image = upload();
    } else { $image = 'error';
    }
    $query = "INSERT INTO content  VALUES('$index1','$title','$keyword','$content','$date','$email','$name','$description','$catagory','$image','$len')";
        ...
        <div class="box">
            <form method="post" enctype="multipart/form-data">
                <div class="form-group">
                    select catagor<select name="catagory" class="form-select">
                    ...
                    select article lenguage<select name="len" class="form-select">
                        <option value="hi">hindi</option>
                        <option value="en">english</option>
                    </select>
                    </br>
                    choose article image(only jpeg or jpg format):
